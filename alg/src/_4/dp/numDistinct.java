...
/**
 * 115. 不同的子序列 - 动态规划
 * 给定一个字符串 S 和一个字符串 T，计算在 S 的子序列中 T 出现的个数。S中组成T的子序列个数
...
public class numDistinct {
    public int numDistinct(String s, String t) {
        ...
        for (int j = 0; j < s.length() + 1; j++) dp[0][j] = 1; //T 为空,因为空集是所有字符串子集, 所以我们第一行都是 1，S中组成T的子序列个数都为1
        for (int i = 1; i < t.length() + 1; i++) {
            for (int j = 1; j < s.length() + 1; j++) {
                if (t.charAt(i - 1) == s.charAt(j - 1)) dp[i][j] = dp[i - 1][j - 1] + dp[i][j - 1];//
                else dp[i][j] = dp[i][j - 1]; //上一个s中的子序列个数
            }
        }
