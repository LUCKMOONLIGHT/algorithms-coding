...
public class coinChange {
    ...
    public int coinChangeI(int[] coins, int amount) {
        ...
        for(int i=0;i<coins.length;i++){  //遍历每一个硬币
            for(int j=coins[i];j<=amount;j++){ //从硬币面值到目标值amount
                if(dp[j - coins[i]] != Integer.MAX_VALUE){ //只有能够凑成dp[j-coin],才能凑成dp[j]
                    dp[j] = Math.min(dp[j], dp[j - coins[i]] + 1);//求最小的凑成个数
                }
            }
        }
    ...
    public int coinChangeII(int[] coins, int amount) {
        ...
        int[] dp = new int[amount + 1];
        dp[0] = 1;//表示0元可以用0元兑换1次，所以 d[0] = 1
        for (int coin : coins) {
            for (int i = coin; i <= amount; i++) { //完全背包问题，顺序遍历
                dp[i] += dp[i - coin]; //d[i] 分配方案应该等于所有 d[i - c] 分配方案之和
            }
        }
        return dp[amount];
    }
