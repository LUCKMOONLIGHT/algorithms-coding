...
解法1：基于partition函数解决。基于第K这个数字，使得比第K个数字小的所有数字都位于数组的左边，比第K个数字大的所有数组都位于数组的右边。（采用这种思路需要将所有数据读入内存，并且修改输入的数组）时间复杂度O（n）

解法2：创建一个大小为K的大根堆来存储最小的k个数字。首先每次从输入的n个整数中读入一个数。如果容器中已有的数字少于k个，则直接把读入的整数放入容器中，否则与根堆比较大小后，将小的放入根堆。(时间复杂度O(nlogn),                                                                                  适合处理海量数据，内存中只需要保存k个数字，每次从磁盘中读取一个数进行比较)
